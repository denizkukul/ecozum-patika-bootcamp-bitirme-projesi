...
export const CreateBoardForm: React.FC<CreateBoardFormProps> = ({ onSubmit }) => {
  ...
  return (
    <>
      {
        active ?
          <Box component='form' onSubmit={handleSubmit} sx={{ height: '200px', width: '200px', m: 4, mt: 0, backgroundColor: 'whitesmoke', borderRadius: '20px' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <Box sx={{ width: '100%', flex: 75, display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flexDirection: 'column' }}>
                <TextField sx={{ width: '80%', my: 2 }} name='title' placeholder='Board Name' value={formValues.title} onChange={updateFormValues} />
                <Button type='submit' >Create</Button>
              </Box>
            </Box>
          </Box> :
          <Button sx={{ height: '200px', width: '200px', m: 4, mt: 0, p: 0, backgroundColor: 'whitesmoke', borderRadius: '20px', ':hover': { backgroundColor: 'lightgray' } }} disableRipple onClick={() => setActive(true)}>
